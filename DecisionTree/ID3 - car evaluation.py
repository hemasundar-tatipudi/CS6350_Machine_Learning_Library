...
train_data = pd.read_csv("DecisionTree/data/car/train.csv", names=column_headers)
test_data = pd.read_csv("DecisionTree/data/car/test.csv", names=column_headers)
...
class DecisionTree:
    ...
    def calculate_gain(self, X, y, attribute):
        if self.criterion == 'information_gain':
            ...
            weighted_error = 0
            for subset_X, subset_y in self.split_dataset(X, y, attribute).values():
                weighted_error += len(subset_y) / len(y) * self.majority_error(subset_y)
            ...
            weighted_gini = 0
            for subset_X, subset_y in self.split_dataset(X, y, attribute).values():
                weighted_gini += len(subset_y) / len(y) * self.gini_index(subset_y)
